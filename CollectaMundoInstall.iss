...
AlwaysRestart=no
UninstallDisplayIcon=.\CollectaMundo\Resources\Images\Icon.ico
...
begin
  ...
  if RegQueryStringValue(HKCU, 'Software\{#MyAppName}', 'InstallLocation', ExistingInstallDir) then
  begin
    if MsgBox(ExpandConstant('{cm:ExistingInstallPrompt}'), mbConfirmation, MB_YESNO) = IDNO then
    begin
      Result := False;  // Cancel the installation if user selects No
    end;
  end;
end;
