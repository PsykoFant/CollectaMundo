...
{
    public class StartupService(IAppSettings settings, IDbConnectionFactory dbFactory, IDatabaseHealthRepository healthRepo, ICardDatabasePreparationService cardDatabasePreparationService, IDatabaseSchemaInitializer schemaInitializer) : IStartupService
    {
        ...
        public async Task EnsureDatabaseIntegrityAsync(StatusViewModel statusVm)
        {
            ...
            if (redownloadDb)
            {
                ...
                //bool downloadSucceeded = await _cardDatabasePreparationService.DownloadResourceAsync(url, path, "Card Database", true, statusVm);
